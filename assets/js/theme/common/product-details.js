...
export default class ProductDetails {
    ...
    getViewModel($scope) {
        return {
            ...
            percentageDiscount: {
                $div: $('.product-discount', $scope),
                $span: $('[data-product-discount]', $scope),
            },
            priceNowLabel: {
    ...
    clearPricingNotFound(viewModel) {
        ...
        viewModel.percentageDiscount.$div.hide();
    ...
    updatePriceView(viewModel, price) {
        ...
        if (price.saved) {
            ...
            if (price.rrp_with_tax) {
                viewModel.percentageDiscount.$div.show();
                viewModel.percentageDiscount.$span.html("-" + Math.round((price.saved.value / price.rrp_with_tax.value) * 100) + "%");
            } else if (price.non_sale_price_with_tax) {
                viewModel.percentageDiscount.$div.show();
                viewModel.percentageDiscount.$span.html("-" + Math.round((price.saved.value / price.non_sale_price_with_tax.value) * 100) + "%");
            }

            if (price.rrp_without_tax) {
                viewModel.percentageDiscount.$div.show();
                viewModel.percentageDiscount.$span.html("-" + Math.round((price.saved.value / price.rrp_without_tax.value) * 100) + "%");
            } else if (price.non_sale_price_without_tax) {
                viewModel.percentageDiscount.$div.show();
                viewModel.percentageDiscount.$span.html("-" + Math.round((price.saved.value / price.non_sale_price_without_tax.value) * 100) + "%");
            }

        } else {
            if (price.non_sale_price_with_tax) {
                viewModel.percentageDiscount.$div.show();
                viewModel.percentageDiscount.$span.html("-" + (100 - (Math.round((price.with_tax.value / price.non_sale_price_with_tax.value) * 100))) + "%");
            } else if (price.non_sale_price_without_tax) {
                viewModel.percentageDiscount.$div.show();
                viewModel.percentageDiscount.$span.html("-" + (100 - (Math.round((price.without_tax.value / price.non_sale_price_without_tax.value) * 100))) + "%");
            }
